...
export const GlareCard = ({
  ...
  const outerSizeClass = fullWidth ? "w-full" : "w-[320px] [aspect-ratio:17/21]";
  return (
    <div
      style={containerStyle}
      className={`relative isolate [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform ${outerSizeClass}`}
      ref={refElement}
      onPointerMove={(event) => {
    ...
    >
      <div className={`h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] ${hideFrame ? '' : 'border border-slate-800'} hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden`}>
